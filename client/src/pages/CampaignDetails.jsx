    ...
    <div>
      ...
      <div className='w-full flex md:flex-row flex-col mt-10 gap-[30px]'>
        <div className='flex-1 flex-col'>
          ...
          <div className='relative w-full h-[5px] bg-[#3a3a43]'>
            <div className='absolute bg-[#4acd8d] h-full' style={{width: `${calculateBarPercentage(state.target, state.amountCollected)}%`, maxWidth:'100%'} }>
            </div>
          </div>
        </div>
      ...
      <div className='mt-[60px] flex lg:flex-row flex-col gap-5'>
        <div className='flex-[2] flex flex-col gap-[40px] '>
          ...
          <div>
            <h4 className='font-epilogue font-semibold text-[18px] text-white  
            uppercase'>Donations </h4>
            
            <div className='mt-[20px] flex flex-col gap-4'>
              {donators.length > 0 ? donators.map((item,index)=>(
                <div key={`${item.donator}-${index}`}
                  className='flex justify-between items-center gap-4'>
                    <p className='font-epilogue font-normal text-[16px] break-all text-[#b2b3bd] leading-[26px]'>
                      {index + 1}. {item.donator}
                    </p>
                    <p className='font-epilogue font-normal text-[16px] break-all text-[#808191] leading-[26px]'>
                       {item.donation}
                    </p>
                </div>
              )) : 
